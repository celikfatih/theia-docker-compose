...
RUN wget -O - https://apt.llvm.org/llvm-snapshot.gpg.key | apt-key add - && \
    echo "deb http://apt.llvm.org/bionic/ llvm-toolchain-bionic-14 main" > /etc/apt/sources.list.d/llvm.list && \
    apt-get update && \
    apt-get install -y \
    clang-tools-$LLVM \
    clangd-$LLVM \
...
RUN apt-get update \
    ...
    && python3.8 -m pip install python-language-server flake8 autopep8
...
RUN git clone https://github.com/opendnssec/SoftHSMv2.git ${SOFTHSM2_SOURCES} && \
    cd ${SOFTHSM2_SOURCES} && \
    git checkout ${SOFTHSM2_VERSION} -b ${SOFTHSM2_VERSION} && \
    ./autogen.sh && \
